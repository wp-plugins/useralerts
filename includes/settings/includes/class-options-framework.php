...
class wdxxx_Options_Framework {
	...
	public function init() {
		...
		add_action( 'admin_init', array( $this, 'wdxxx_set_theme_option' ) );
	 ...
	 * @since 1.7.0
	 */
	function wdxxx_set_theme_option() {

		// Load settings
        $optionsframework_settings = get_option( 'optionsframework' );

        // Updates the unique option id in the database if it has changed
        if ( function_exists( 'wdxxx_optionsframework_option_name' ) ) {
			wdxxx_optionsframework_option_name();
        }
        elseif ( has_action( 'wdxxx_optionsframework_option_name' ) ) {
			do_action( 'wdxxx_optionsframework_option_name' );
        }
	...
	static function &_optionsframework_options() {
		...
		if ( !$options ) {
	        ...
	        $location = apply_filters( 'wdxxx_Options_Framework_location', array('options.php') );
